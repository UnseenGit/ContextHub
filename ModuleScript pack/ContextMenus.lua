...
for i = 1, 1032 do
	...
	if Color and not ColorLibraryArray.Colors[Color.Name] then
		ColorLibraryArray.Colors[Color.Name] = Color.Color
	end
end
	...
	Prompts = {
		...
		Content = {
			...
			TextLabel = {
				...
				RichText = true,
				Text = "If you see this the dev done messed up.",
				TextSize = 14,
				TextXAlignment = "Left",
				TextYAlignment = "Top",
				AutoLocalize = false,
...
local function SetRecent(Name)
	local Recent = ColorLibraryArray.Recent
	ColorLibraryArray.Recent = { Name }
	for _, Name in pairs(Recent) do
		if not table.find(ColorLibraryArray.Recent, Name) then
			table.insert(ColorLibraryArray.Recent, Name)
		end
		if #ColorLibraryArray.Recent > 4 then
			break
		end
	end
end
function module.OpenColorPicker(CurrentColor, Parent)
	...
	local succ, err = pcall(function()
		...
		LibraryButton.MouseButton2Click:Connect(function()
			if #ColorLibraryArray.Recent > 0 then
				local Square = "■"
				local RecentColors = {}
				for _, Color in pairs(ColorLibraryArray.Recent) do
					if ColorLibraryArray.Colors[Color] then
						table.insert(RecentColors, {
							Text = '<font color="#'
								.. ColorLibraryArray.Colors[Color]:ToHex()
								.. '">'
								.. Square
								.. "</font> "
								.. Color,
							M1Func = function()
								ColorValue = ColorLibraryArray.Colors[Color]
								SetPickerPos()
								SetRecent(Color)
							end,
						})
	...
	if err then
		error("[*ERROR*] ColorPicker Error: " .. err)
	end
	local ret = false
...
function module.OpenColorLibrary(AdditionalSettings, parent, SetColorFunction)
	...
	for Name, Color in pairs(ColorLibraryArray.Colors) do
	...
	local succ, err = pcall(function()
		...
		if TooltipT then
			...
			GuiObject.MouseMoved:connect(function()
				...
				if err then
					error("[*ERROR*]" .. "CreateTooltip Error: " .. GuiObject:GetFullName() .. ":" .. err)
				end
			end)
		end
	end)
